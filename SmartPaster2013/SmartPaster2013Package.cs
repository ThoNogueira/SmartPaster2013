...
namespace SmartPaster2013
{
    ...
    [Guid(GuidList.guidSmartPaster2013PkgString)]
    public sealed class SmartPaster2013Package : Package
    {
        /// <summary>
        ...
        protected override void Initialize()
        {
            ...
            if (null != mcs)
            {
                ...
                var menuCommandVerbatimID = new CommandID(GuidList.guidSmartPaster2013CmdSet, (int)PkgCmdIDList.cmdidPasteAsVerbatimString);
                var menuItemVerbatim = new MenuCommand(CallPasteAsVerbatimString, menuCommandVerbatimID);
                mcs.AddCommand(menuItemVerbatim);

                var menuCommandStringID = new CommandID(GuidList.guidSmartPaster2013CmdSet, (int)PkgCmdIDList.cmdidPasteAsString);
                var menuItemString = new MenuCommand(CallPasteAsString, menuCommandStringID);
                mcs.AddCommand(menuItemString);

                var menuCommandBytesID = new CommandID(GuidList.guidSmartPaster2013CmdSet, (int)PkgCmdIDList.cmdidPasteAsBytes);
                var menuItemBytes = new MenuCommand(CallPasteAsBytes, menuCommandBytesID);
                mcs.AddCommand(menuItemBytes);

                var menuCommandSbID = new CommandID(GuidList.guidSmartPaster2013CmdSet, (int)PkgCmdIDList.cmdidPasteAsStringBuilder);
